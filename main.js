...
// ------------------------------------------------------------下面是移动的tab切换
var MoveTab1 = Class.extend({ //tab切换
    bind:function (that) {
        var html = ''
        html +='<div class="t-tab-content"><div class="t-tab-bg" ></div>'
        for (var i = 0; i < that.arr.length; i++) {
            html +='<div class="t-tab-material" data-Id="'+that.arr[i].id+'">'+that.arr[i].name+'</div>'
        }
        html +='</div>'
        var index=1;
        $('#'+that.id).html(html)
        $('.t-tab-material').eq(0).css('color','white')
        if (that.event=='click') {
            $('.t-tab-material').mouseover(function(){
                $('.t-tab-material').css('color','#94b893')
                $('.t-tab-bg').css('left',( $(this).index()-1)*84+'px');
                $(this).css('color','white')
            })
            $('.t-tab-material').mouseout(function(){
                $('.t-tab-bg').css('left',(index-1)*84+'px');
                $('.t-tab-material').css('color','#94b893')
                $('.t-tab-material').eq(index-1).css('color','white')
            })
            $('.t-tab-material').on('click',function(){
                index = $(this).index()
                that.success($(this).data().id)
            })
        }else{
            $('.t-tab-material').mouseover(function(){
                $('.t-tab-material').css('color','#94b893')
...
var Accordion = Class.extend({ //手风琴插件
    bind:function (that) {
        var html = ''
        for (var i = 0; i < that.arr.length; i++) {
            html+='<div class="t-accordion-content">'
            html+='<div class="t-accordion-title"  data-id="'+that.arr[i].id+'">'+that.arr[i].title+'</div>'
            html+='<div class="t-accordion">'+that.arr[i].content+'</div>'
            html+='</div>'
        }
        $('#'+that.id).html(html)
        $('.t-accordion-title').on('click',function(){
            if ($(this).siblings().hasClass('on')) { //open状态
                $(this).siblings().slideUp(400).removeClass("on")
                that.success({type:1,id:$(this).data().id})
            }else{ //关闭状态
                $(this).parent().siblings().children('.t-accordion').removeClass("on")
                $(this).parent().siblings().children('.t-accordion').slideUp(500)
                $(this).siblings().slideDown(400)
                $(this).siblings().addClass("on")
                that.success({type:2,id:$(this).data().id})
            }
        })
    },
    init: function (options) {
...
var ChiTu = (function () {
    return {
        ...
        MoveTab:function (res) {//移动的tab
            return new MoveTab1(res);
        },
        Accordion: function(res) {//手风琴插件
            return new Accordion(res);
        }
    }
}());
